...
class GRTC extends EventEmitter {
    ...
    dataHandler(data) {
        ...
        if ('publicKey' in parsedData) {
            ...
            self.emit('secretAck', parsedData);
        }else {
    ...
    securityHandler() {
        ...
        return new Promise((resolve, reject) => {
             ...
             * Ack the initiator that secret is received.
             */
            self.on('secret', (sharedKey) => {
                self.emit('peerSecret', crypto.decrypt(sharedKey, self.keys.privateKey));
    ...
    /**
     * Transport Layer after session is encrypted.
     * Called after secretAck is received.
     */
    transportLayer() {
        console.log('transport layer starting established');
    }
    ...
    init() {
        ...
        self.on('secretAck', self.transportLayer);
    }
}
